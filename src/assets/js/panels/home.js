    ...
    async initLaunch() {
        document.querySelector('.play-btn').addEventListener('click', async () => {
            ...
            launch.on('progress', (progress, size) => {
                ...
                document.querySelector(".text-download").innerHTML = `Descargando cliente: ${((progress / size) * 100).toFixed(0)}%`
            ...
            launch.on('check', (progress, size) => {
                progressBar.style.display = "block"
                document.querySelector(".text-download").innerHTML = `Validando archivos ${((progress / size) * 100).toFixed(0)}%`
                progressBar.value = progress;
                progressBar.max = size;
            });
            ...
            launch.on('patch', patch => {
                console.log(patch);
                info.innerHTML = `Abriendo cliente...`
            });
